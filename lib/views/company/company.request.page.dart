                ...
                return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                  ...
                  builder: (context, s) {
                    ...
                    docs.sort((a, b) {
                      ...
                      final da = ta?.toDate();
                      final db = tb?.toDate();
                      return (db?.millisecondsSinceEpoch ?? 0)
                      ...
                      return Center(
                        child: Column(
                          ...
                          children: const [
                            ...
                            SizedBox(height: 12),
                            Text('Nenhuma solicitação pendente', style: TextStyle(fontSize: 16)),
                          ],
                        ),
                      );
                    }
                    return ListView.builder(
                      ...
                      itemBuilder: (context, i) {
                        ...
                        final item = {
                          ...
                          'data': '${x['dateText'] ?? ''} ${x['startTime'] ?? ''} - ${x['endTime'] ?? ''}',
                          'imagem': x['characterPhotoUrl'] ?? '',
                        };
                        return Request(
                          item: item,
                          onDecline: () async {
                            await FirebaseFirestore.instance.collection('requests').doc(d.id).update({'status': 'declined'});
                          ...
                          onAccept: () async {
                            ...
                            await FirebaseFirestore.instance.collection('appointments').add({
                              ...
                              'characterPhotoUrl': data['characterPhotoUrl'],
                            ...
                            await FirebaseFirestore.instance.collection('requests').doc(d.id).update({'status': 'accepted'});
                            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Solicitação aceita e adicionada aos agendamentos')));
                          },
                        );
                      },
                    );
